...
- [x] `git reflog`로 HEAD 이동 기록을 확인했다
- [x] reflog에서 사라진 커밋의 해시를 찾아냈다
- [x] `git reset --hard`로 main의 커밋을 복구했다 (3개 → 5개)
- [x] `git branch`로 삭제된 feature/payment를 복구했다
- [x] 복구된 브랜치의 파일이 정상인지 확인했다
- [ ] (심화) reflog의 시간 정보를 확인했다
- [x] `git log`와 `git reflog`의 차이를 이해했다
